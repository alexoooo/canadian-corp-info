...
The audience is narrow: owners of a Canadian-controlled private corporation (CCPC), typically holding stocks/ETFs in a corporate trading account. Keep that frame — do not generalize content to personal tax, US filers, or other entity types unless the existing document already does. Sole-proprietor treatment has one sanctioned home: the side group `guide/Sole-Proprietorship/` (contrast and on-ramp for the unincorporated contractor, listed last in the README). Outside that group, a brief contrast with sole-proprietor or personal-tax treatment is allowed where it clarifies the corporate treatment and stays clearly secondary (see `guide/Paying-Yourself/Owner-Corporation-Transactions.md`); link into the group rather than growing the contrast in place. General personal-tax coverage (employment income, credits, benefits) stays out everywhere.

---

## Document architecture

`README.md` is the topic index. `guide/` is organized into eight thematic group folders — `Overview/`, `Corporate-Lifecycle/`, `Bookkeeping/`, `Operations/`, `Paying-Yourself/`, `Investments/`, `Filing-And-CRA/`, and `Sole-Proprietorship/` — each opening with a hub page named after the folder (e.g. `Operations/Operations.md`) that introduces and indexes the group. Each page within a group is a standalone topic; the pages form a dependency graph rather than a flat collection. A topic that has multiple files (parent page + sub-page, or page + screenshots) nests in its own subfolder within its group so related files stay co-located.
...
- **`guide/Sole-Proprietorship/Sole-Proprietorship.md`** — hub for the sole-proprietorship side group (the unincorporated contractor); the guide's only sanctioned personal-tax territory, kept to the delta from the corporate pages
