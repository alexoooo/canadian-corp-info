...
- It is assumed that the shareholder is a Canadian resident, and is not personally connected to the trust which issued the T3
...
T3 Box 26 is `other income` paid to a beneficiary of a trust.  
...
- `Canaidan-source` domestic payer, vs `foreign-source` foreign payer
- `trust` issuer with T3, vs `corporation` that issues T5

T3 Box 26 is a "catch-all" for `passive` investment income from a `trust` that is 100% taxable as `ordinary income` from a `Canaidan-source`.  
Note that the type of slip you receive depends on the type of issuer, a `corporation` paying `passive` `ordinary income` from a `Canaidan-source` results in a T5 (rather than T3).  

There are a few types of income that can show up in T3 Box 26:
...
- Other income generated by the trust (e.g. an income trust that owns an operating company)
...
- For example in Ontario, the combined rate on AII is approximately 50.17%, whereas the SBD rate is 12.2%
...
What the CRA says about T3 Box 26 tax treatment:
- It must be included in corporate income: roll up to GIFI 8299 on "Schedule 125 - Income statement information"
- It is correctly incorporated as net income from property: Box 32 in Part 1 of Schedule 7 to calculate "Aggregate Investment Income" (AII)

For the GIFI code:
- GIFI 8299 (Revenue) is a calculation, CRA does not mandate a specific GIFI code
- It is generally accepted that the following are reasonable: GIFI 8090 (Investment revenue), and GIFI 8230 (Other revenue)
