...
**Who this is for**: owners of a Canadian-controlled private corporation (CCPC) that buys goods for resale (retail, e-commerce, parts-and-services) or that buys materials for use in its own operations, and who need to translate those purchases into ledger entries, a year-end valuation, the cost-of-sales section of T2 Schedule 125, and the inventory lines of Schedule 100.

**TLDR**:
- *Inventory* is property held for sale, held for use in producing property held for sale, or whose cost is relevant in computing income for the year (ITA [s.248(1)](https://laws-lois.justice.gc.ca/eng/acts/I-3.3/section-248.html))
- A property is either *inventory* or *depreciable property* — never both at the same time; the boundary is what the corp acquired the property *for*
...
- *Cost of sales* (COGS) is the matching deduction at the moment a unit is sold; nothing is amortized over time the way *Capital Cost Allowance* (CCA) is for fixed assets
...
The test for which category a property falls into is the *purpose at acquisition*:
...
- Bought as a one-off speculation that you intend to flip: *adventure or concern in the nature of trade*, taxed at cost under ITA [s.10(1.01)](https://laws-lois.justice.gc.ca/eng/acts/I-3.3/section-10.html) rather than at LCM — also inventory in form, but with cost-only valuation

If purpose changes mid-stream (say a delivery van that was bought to resell, taken off the lot, and pressed into operational use), that is a *change in use* event. The deemed-disposition rules in s.45 / s.13(7) apply and trigger a transfer between inventory and a CCA class at fair market value. This is rare for a typical CCPC and is out of scope here.
...
For a *manufacturing* CCPC, conversion costs (direct labour, factory overhead) would also enter inventory cost under accounting standards. ITA s.10 accepts the cost figure that a properly applied accounting standard produces, and CRA generally accepts ASPE Section 3031 / IFRS IAS 2. Manufacturing cost build-up is out of scope here; the worked examples below assume buy-and-resell or buy-for-own-use.


## Materials versus finished items

The distinction the law cares about is between two destinations for purchased materials:
...
This boundary matters because the deduction *timing* is completely different:
- Inventory expensed at sale: the deduction follows the matching revenue; no time-shape
...
- The earliest of: first use to earn income, capable of producing the intended service, end of the second tax year after acquisition (the rolling-two-year / 357-day rule under s.13(27))
...
- Pointer only — see [HST](HST.md) for the full mechanics


## Tracking through the year

The page assumes a *perpetual* inventory system — the inventory ledger is updated at each purchase and each sale, and the running balance is always current. A *periodic* system (no per-transaction update, COGS plugged at year-end from a physical count) works but obscures shrinkage and complicates LCM testing. Modern e-commerce platforms (Shopify, WooCommerce) produce perpetual inventory by default; spreadsheet tracking can do either.
...
- Reconcile in-transit goods using the *FOB terms*: FOB shipping point means title transferred at the supplier's dock, so the corp owns the goods in transit and they belong in closing inventory; FOB destination means title transfers on receipt, so in-transit goods do not yet belong
...
- `8450 Other direct costs`: a catch-all — packaging, freight to a customer if treated as a cost rather than as a selling expense (depends on the corp's policy), inventory shrinkage if not booked to 8518
...
- Exceptions: a change in valuation method approved by CRA (the catch-up may be on S1); accounting-vs-tax LCM divergence (rare under ASPE/IFRS, both of which broadly track ITA s.10 LCM); a write-down that the accountant booked but for which CRA disallowed the FMV evidence on audit (also rare)

A note on the CIP / fixed-asset boundary:
...
This is the user's lumber-and-shed scenario; the goal of the example is to show that the materials are *not* inventory, that no deduction flows during construction, and that CCA starts only on completion and available-for-use.
...
- The choice between the two paths is made at acquisition, by the corp's intent for the materials
