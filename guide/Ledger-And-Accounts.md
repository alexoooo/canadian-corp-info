...
See [Inventory and COGS](Cost-Recovery/Inventory-And-COGS.md).  
...
For the Quick Method's effect on the booked amounts, see [HST](HST.md#quick-method) and [Expense Classification](Expense-Classification.md#hst-and-the-booked-amount).  
...
`HST collected` ($1,040) net of `HST receivable` ($390) leaves $650 owing to CRA — the net HST the regular-method return remits; see [HST](HST.md).  
...
- *Cost of sales under a periodic system*: with no per-sale inventory update, COGS is plugged at year-end as opening inventory + purchases − closing inventory (from the count); `8518 Cost of sales` is that plug; see [Inventory and COGS](Cost-Recovery/Inventory-And-COGS.md)
- *Whole-dollar rounding residual*: rounding each line to the dollar can throw a subtotal off by a dollar or two, so one designated line (retained earnings `3849` on the balance sheet, net income on the income statement) absorbs the residual; see [Whole-dollar rounding](Whole-Dollar-Rounding.md)
