...
This account naming is the canonical convention used in the worked examples elsewhere.  
...
- ITCs on operating inputs are *not* claimed
  - `HST receivable` carries only ITCs on capital purchases (see [Quick Method](#quick-method) below)
...
Remittance rate:
- Depends on three things (the rate matrix in RC4058 has roughly 30 cells)
...
- Under a *Quick Method* election this holds only for imported capital property
  - Import HST on operating inputs earns no ITC (see [Quick Method](#quick-method))
