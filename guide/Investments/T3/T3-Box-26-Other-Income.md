...
**Who this is for**:
- Owners of a Canadian-controlled private corporation (CCPC) who receive a T3 with box 26 amounts
...
Limitations:
- This document only covers ETFs structured as a trust
  - Other types of investments (e.g. direct real estate holdings or non-ETF trust investments) are out of scope
  - They might have different or additional rules
...
- *Ordinary* income taxed at 100% vs *preferential* treatment as capital gains or dividends
  - Preferential: capital gains inclusion rate; dividend gross-up and credit
...
Active business carried on by the trust can also appear in Box 26.  
That does not apply to an ETF structured as a mutual fund trust.  
A mutual fund trust cannot carry on a non-investment business without losing that status.  

In general, the T3 Box 26 amount is a residual bucket and can represent both active and passive income:
- The treatment as Active Business Income (ABI) vs Aggregate Investment Income (AII) has tax consequences
- For a publicly traded ETF structured as a trust, Box 26 is typically AII:
  ...
  - Some checks you can perform that increase the likelihood that the T3 Box 26 amount is AII:
    ...
    - Download the PDF
    - Confirm that the T3 Box 26 amount matches what is labelled "Other Income (Investment Income)"
    - You can also locate the prospectus, search for "Income Tax Considerations" or "Taxation of the Fund"
      - Confirm it qualifies as a "mutual fund trust"
    - A mutual fund trust is meant to be an investment vehicle
      - It generally cannot carry on a non-investment business if it wants to maintain that status
    - You can look for any T3 slip attachments or issuer tax notes related to T3 Box 26
      - Typically published on the ETF sponsor's website alongside the annual tax breakdown
      - For example under "Tax Information" or "Distributions"
    - These items support the conservative filing position for a plain-vanilla ETF
      - They are not by themselves a binding CRA determination
  - There is usually no practical basis to treat Box 26 as ABI absent unusually strong issuer-specific facts
- In rare fact-specific situations, the AII result can be different
  - That is outside the plain-vanilla ETF scope covered here
- If the T3 Box 26 amount is reported as AII, but arguably should have been treated as ABI:
  - This is a legally conservative filing position, but it can mean paying more tax than necessary
  - For simplicity, in this document we assume the amount is AII
    - This can result in paying slightly more tax than the alternative
...
- Other treatment of T3 Box 26 is outside the scope of this document
  - For example, from a trust with unusual non-plain-vanilla facts
...
- The same broad economic idea of "ordinary income" does not by itself determine the slip type
  - The issuer's legal structure is significant
...
- It does not benefit from the Capital Gains Inclusion Rate (Box 21) or dividend tax treatment (Box 49)
  - It does not change the cost base (Box 42)
...
- Generally:
  ...
  - ITA s.108(5)(a) generally deems those trust-beneficiary amounts to be income from a property
    - The property being an interest in the trust
  - ITA s.129(4) also carves out income that would not have been income from property but for ITA s.108(5)(a)
    - So not every Box 26 amount from every trust necessarily feeds AII and NERDTOH in the same way
- For a vanilla index ETF that is structured as a mutual fund trust:
  ...
  - If an amount is part of AII for a CCPC, it can contribute to Non-Eligible Refundable Dividend Tax on Hand (NERDTOH)
    - Through the broader formula in ITA s.129(4)
  - Adjusted aggregate investment income (AAII) over $50,000 grinds the small business deduction under ITA [s.125(5.1)](https://laws-lois.justice.gc.ca/eng/acts/I-3.3/section-125.html)
    - Every dollar of AAII above $50,000 reduces the $500,000 SBD business limit by $5
      - The limit is fully eliminated at $150,000 of AAII
    - For a plain-vanilla ETF holding, AAII is AII plus the ETF's Canadian portfolio dividends
    - Active business income above the reduced limit is taxed at the general corporate rate, not the small-business rate
  - The portion of Part I tax on AII that flows into NERDTOH (30⅔% of AII per s.129(4))
    - Recovered only when the corporation pays a non-eligible dividend
    - See [ERDTOH-NERDTOH.md](../../Paying-Yourself/Dividends/ERDTOH-NERDTOH.md) for refund mechanics
...
- Enter it as part of net income from property: Box 32 in Part 1 of Schedule 7 (S7)
  - This calculates "Aggregate Investment Income" (AII)
- For Box 26 from a trust that is not a mutual fund trust, the AII classification and S7 treatment may differ
  - For example certain commercial or income trusts, or a trust with active business income
  - Out of scope here
...
- The following could all be consistent with CRA guidelines:
  ...
  - GIFI 8094 (Interest from other Canadian sources), which rolls up to GIFI 8090
    - Only when the T3 Box 26 amount is actual interest (e.g. bond index ETF like ZAG)
- This document uses GIFI 8090 (Investment revenue), with a subaccount "Other investment income" (coded 8090-2)
- If the corporation also earns direct interest income (e.g. GIC interest, savings account interest):
  - Use GIFI 8094 for that amount and GIFI 8090 (ledger subaccount 8090-2) for the T3 Box 26 amount
  - This keeps the Schedule 125 reconciliation cleaner and avoids mixing T3-sourced income with directly earned interest
...
We also add subaccounts where the codes are not granular enough (with -&lt;suffix&gt;).  
When filing GIFI coded financial statements, each code is considered individually.  
For example: 8090 is separate from 8094, even though 8094 is under 8090 in the hierarchy.  
...
Since CRA does not require specific ledger entries, here we adopt a particular bookkeeping convention.  
It would generally be considered reasonable.  
...
A distribution can be properly receivable before year-end with the cash paid in the following year.  
In that case, a receivable entry can be used for an accrual.  
...
S7, assuming the classification is passive property income (a publicly traded ETF structured as a mutual fund trust):
...
For the corporate-tax framing of where T3 income lands, see [Small-Business-Tax.md](../../Overview/Small-Business-Tax.md).  
