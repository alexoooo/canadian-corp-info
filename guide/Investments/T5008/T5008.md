...
For a US-dollar security, compute the gain in Canadian dollars: translate the proceeds at the trade-date rate (a settlement-date rate is also defensible) and each ACB lot at its own acquisition-date rate, so the exchange-rate move between buying and selling is itself part of the gain or loss; see [Foreign Currency](../../Bookkeeping/Foreign-Currency/Foreign-Currency.md).  
...
A capital gain is only half taxable: Schedule 6 reports the full gain, and Schedule 1 line 113 (Taxable capital gains from Schedule 6) carries the taxable half into income — FutureTax fills it from S6 automatically.  
Because the ledger entry above books the whole gain, Schedule 1 also backs out the full book gain — on line 401 (Gain on disposal of assets per financial statements), or equivalently an Other-deductions entry (description 705 / amount 395) at the full amount — so taxable income reflects only the 50% inclusion.  
Backing out only the non-taxable half would double-count once line 113 is filled.  
That non-taxable half also adds to the corporation's *Capital Dividend Account* (CDA), which the corp can later pay out to shareholders tax-free; see [Capital Dividend Account](../Capital-Dividend-Account/Capital-Dividend-Account.md).
