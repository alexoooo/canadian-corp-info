...
**Who this is for**:
- Owners of a Canadian-controlled private corporation (CCPC) who get a T5008 from selling ETF or stock investments

Limitations:
- Covers stocks and ETFs sold in a corporate brokerage account
  - Other instruments (real estate, etc.) are out of scope and might have different or additional rules
...
- Box 21: Proceeds of disposition or settlement amount
  ...
  - Schedule 6 computes the gain as proceeds − ACB − outlays and expenses, so the sell-side commission is deducted
    exactly once; take one of these two branches and stay inside it:
    - *Box 21 is gross*: enter the gross figure as Schedule 6 proceeds and the commission in the outlays and
      expenses column
    - *Box 21 is net*: enter it as proceeds and claim no separate outlay — the commission is already out
  - Reducing proceeds *and* entering the outlay double-counts it; entering gross proceeds with no outlay drops it
...
In addition, you will also need the earliest date of continuous ownership of the security (entered in S6 / Part 1).  
That is the date of the first purchase that contributed to the current pooled position.  
It is tracked in your ACB spreadsheet; see [Adjusted-Cost-Base-Tracking.md](../Adjusted-Cost-Base/Adjusted-Cost-Base-Tracking.md).  
...
Your brokerage account should promptly display the new cost base amount (for sales).  
T3-driven adjustments such as ROC and phantom distributions may lag until March/April; see T3.md.  

When you buy, the brokerage commissions increase ACB.  
They are capitalized into ACB rather than entered separately on Schedule 6.  
When selling, the commission is an *outlay or expense of disposition*: it is not part of the ACB, and it reduces the
gain either through the outlays column or through a net Box 21 — one or the other, never both.  

For a US-dollar security, compute the gain in Canadian dollars.  
Translate the proceeds at the trade-date rate, the convention this guide applies to every securities event.  
Any difference on settlement is a separate foreign-exchange result, not an alternative translation basis for the
disposition.  
Translate each ACB lot at its own acquisition-date rate.  
The exchange-rate move between buying and selling is itself part of the gain or loss; see [Foreign Currency](../../Bookkeeping/Foreign-Currency/Foreign-Currency.md).  


## T2 - S6

When you sell an investment, report it in "S6 - Summary of Dispositions of Capital Property" of the T2.  
This is in addition to the above ledger entries.  
Report the ACB from your own records.  
Do not use T5008 Box 20 (cost or book value), which may differ from your calculated ACB.  
A disposition can occur with no T5008 received (rare, but possible).  
In that case, report the sale on S6 using your own trade records and independently calculated ACB.  
...
- You can determine if an investment is a stock or trust by looking at T5008 Box 15
  - "SHS" for shares and "UNT" for units
- Sometimes this is not included, and you can check the prospectus or CDS filing
  - Most Canadian-listed equity ETFs are Trust units
- If you use the wrong section, the capital gains tax will be the same
  - There could be other implications (out of scope for this document)
...
For pooled securities, you can use the date of the very first purchase that contributed to the current pool.  
This date convention is conservative in the absence of specific CRA guidance.  
...
For a partial sale (e.g. selling 60 of 100 units), the per-unit ACB is unchanged.  
Only the Remaining Quantity and total ACB decrease by (units sold × per-unit ACB).  
...
- Column 140: 7 Outlays and expenses from disposition = the sell-side commission, but only on the gross-Box-21
  branch; leave it blank when Box 21 is already net
...
- Column 440: 5 Outlays and expenses from disposition = same rule as Part 1 above

Tie the ledger to whichever branch you chose, so the booked proceeds and the filed proceeds agree.  


## Taxable Half and the CDA

A capital gain is only half taxable: Schedule 6 reports the full gain.  
Schedule 1 line 113 (Taxable capital gains from Schedule 6) carries the taxable half into income.  
FutureTax fills it from S6 automatically.  
Because the ledger entry above books the whole gain, Schedule 1 also backs out the full book gain.  
Line 401 (Gain on disposal of assets per financial statements) does this; FutureTax fills it from GIFI 8211.  
Add no other-deductions row for the same gain, which would remove it a second time.  
Taxable income then reflects only the 50% inclusion (see [Schedule 1 Lines](../../Filing-And-CRA/T2-Schedules.md#schedule-1-lines)).  
That non-taxable half also adds to the corporation's *Capital Dividend Account* (CDA).  
The corp can later pay it out to shareholders tax-free; see [Capital Dividend Account](../Capital-Dividend-Account/Capital-Dividend-Account.md).  
