...
The difference between the BoC mid-rate and the rate the bank used is the *implicit spread*.  
The spread is how the bank earns on the conversion.  

The simplest case is a plain CAD↔USD transfer between two accounts at the same bank.  
A USD wire from a customer landing in the CAD account also closes out a USD receivable.  
That combined case is covered in [Getting paid in USD](Getting-Paid-In-USD.md).  

Bookkeeping treatment under the multi-currency convention:
- The transfer is an FX event between a CAD-native and a USD-native account; each leg balances in its own currency
  - `FX gain/loss - CAD` (8231-1) and `FX gain/loss - USD` (8231-2) are the per-currency bridge accounts
- No FX gain or loss is recognized at the moment of transfer
  - The trading accounts accumulate the per-currency positions until period-end revaluation at the closing BoC rate
- The implicit spread surfaces at period-end revaluation
  - Until then, the bank's actual settlement rate is the effective "cost" of the USD position
...
- [Norbert's Gambit](Norberts-Gambit.md) through a discount broker: ~15–20 basis points of bid-ask spread, plus flat per-trade commissions
  - The spread spans the two legs; because the commissions are fixed, the all-in cost depends on the amount converted
  - Break-even against the bank spread is worked on the sub-page
...
- Setup: corp moves CAD 10,000 from `Deposits` (1002-1, CAD-native) to `Deposits - USD` (1003, USD-native)
  - Both accounts are at the same Canadian bank; the transfer settles Apr 20
...
- No FX gain or loss is recognized at this point; both trading accounts now hold per-currency positions
  - 8231-1 carries CAD 10,000 debit; 8231-2 carries USD 7,210 credit
- The implicit spread of ~CAD 194 surfaces at period-end revaluation:
  - If year-end BoC rate is 1.36 and the USD 7,210 is still held, translate 8231-2 at 1.36
